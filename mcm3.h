...
class mcm3:public model
{
    public:
        double forward[DOCPERPAGE+2][2],backward[DOCPERPAGE+2][2];
        double gamma1[DOCPERPAGE+2][DOCPERPAGE+2][DOCPERPAGE+2],gamma0[DOCPERPAGE+2][DOCPERPAGE+2][DOCPERPAGE+2],gamma[DOCPERPAGE+2][DOCPERPAGE+2][DOCPERPAGE+2];
        //alpha_cnt=train_cnt
        ...
        void train_clear()
        {
            ...
            for(int i=0;i<=DOCPERPAGE;++i)
                for(int j=0;j<=DOCPERPAGE;++j)
                    for(int k=0;k<=DOCPERPAGE;++k)
                        gamma1[i][j][k]=gamma0[i][j][k]=.0001;
        }
        void train()
        {
            for(int i=0;i<docs.size();++i)
            ...
            for(int round=1;round<=MAXROUND;++round)
            {
                ...
                for(auto &sess:sessions)
                {
                    ...
                    for(int i=1;i<=DOCPERPAGE;++i)
                    {
                        ...
                        if(sess.click_time[i]>.1)
                        {
                            ...
                            beta1[vid]+=1;
                            //beta0[vid]+=1;
                            gamma1[i][i-last_clk][clk_cnt]+=1;
                            //gamma0[i][i-last_clk]+=;
                            //s_c1[id]+=forward[i-1][0]*gamma[i][i-last_clk]*alpha[id]*beta[vid]*backward[i][1];要转成条件概率计算
                            s_c1[id]+=forward[i-1][0]*gamma[i][i-last_clk][clk_cnt]*alpha[id]*beta[vid]*s_c[id]*backward[i][1]/prob_cs;
                            s_c0[id]+=forward[i-1][0]*gamma[i][i-last_clk][clk_cnt]*alpha[id]*beta[vid]*(1.-s_c[id])*backward[i][0]/prob_cs;
                        ...
                        {
                            alpha1[id]+=(forward[i-1][0]*alpha[id]*backward[i][0]*(1.-gamma[i][i-last_clk][clk_cnt])\
                                +forward[i-1][1]*backward[i][1]*alpha[id]+forward[i-1][0]*gamma[i][i-last_clk][clk_cnt]*alpha[id]*\
                            ...
                            beta1[vid]+=beta[vid]*(forward[i-1][0]*backward[i][0]*(1.-alpha[id]*gamma[i][i-last_clk][clk_cnt])+forward[i-1][1]*backward[i][1])/prob_cs;
                            beta0[vid]+=((1.-beta[vid])*(forward[i-1][0]*backward[i][0]*(1.-alpha[id]*gamma[i][i-last_clk][clk_cnt])+forward[i-1][1]*backward[i][1])+\
                                forward[i-1][0]*gamma[i][i-last_clk][clk_cnt]*alpha[id]*(1.-beta[vid])*(s_e[id]*backward[i][1]+(1.-s_e[id])*backward[i][0]))/prob_cs;
                            //beta0[vid]+=1.-beta[vid]*(forward[i-1][0]*backward[i][0]*(1.-alpha[id]*gamma[i][i-last_clk])+forward[i-1][1]*backward[i][1])/prob_cs;
                            gamma1[i][i-last_clk][clk_cnt]+=(forward[i-1][0]*backward[i][0]*gamma[i][i-last_clk][clk_cnt]*(1.-alpha[id]+alpha[id]*(1.-beta[vid])*(1.-s_e[id]))+\
                                forward[i-1][0]*backward[i][1]*gamma[i][i-last_clk][clk_cnt]*alpha[id]*(1.-beta[vid])*s_e[id])/prob_cs;
                            gamma0[i][i-last_clk][clk_cnt]+=(forward[i-1][0]*backward[i][0]*(1.-gamma[i][i-last_clk][clk_cnt]))/prob_cs;
                            /*double w=0;
                            ...
                            */
                            //s_c1[id]+=;
                            //s_c0[id]+=;
                            s_e1[id]+=(forward[i-1][0]*gamma[i][i-last_clk][clk_cnt]*alpha[id]*(1.-beta[vid])*s_e[id]*backward[i][1])/prob_cs;
                            s_e0[id]+=(forward[i-1][0]*gamma[i][i-last_clk][clk_cnt]*alpha[id]*(1.-beta[vid])*(1.-s_e[id])*backward[i][0])/prob_cs;
                        }
                    }
                }
                for(int i=0;i<docs.size();++i)
                ...
                for(int i=0;i<=DOCPERPAGE;++i)
                    for(int j=0;j<=i;++j)
                        for(int k=0;k<=i;++k)
                        {
                            gamma[i][j][k]=gamma1[i][j][k]/(gamma0[i][j][k]+gamma1[i][j][k]);
                        }
        ...
        void get_prob(Session &sess)
        {
            ...
            forward[0][0]=1.0;
            forward[0][1]=0.0;
            int last_clk[DOCPERPAGE+2],clk_cnt[DOCPERPAGE+2];
            clk_cnt[0]=0;
            ...
            for(int i=0;i<DOCPERPAGE;++i)
            {
                ...
                {
                    forward[i+1][0]=forward[i][0]*gamma[i+1][i+1-last_clk[i]][clk_cnt[i]]*alpha[id]*beta[vid]*(1.-s_c[id]);
                    forward[i+1][1]=forward[i][0]*gamma[i+1][i+1-last_clk[i]][clk_cnt[i]]*alpha[id]*beta[vid]*s_c[id];
                    //cerr<<"CLK!\t"<<sess.doc_id[i+1]<<"\t"<<vid<<"\t"<<forward[i+1][0]<<"\t"<<forward[i+1][1]<<"\t"<<gamma[i+1][i+1-last_clk[i]]<<"\t"<<alpha[id]<<"\t"<<beta[vid]<<endl;
                    last_clk[i+1]=i+1;
                    if(vid==1)
                        clk_cnt[i+1]=clk_cnt[i]|1;
                    else
                        clk_cnt[i+1]=clk_cnt[i]|2;
                }
                else
                {
                    forward[i+1][0]=forward[i][0]*(1.+gamma[i+1][i+1-last_clk[i]][clk_cnt[i]]*alpha[id]*((1.-beta[vid])*(1.-s_e[id])-1.));
                    forward[i+1][1]=forward[i][0]*gamma[i+1][i+1-last_clk[i]][clk_cnt[i]]*alpha[id]*(1.-beta[vid])*s_e[id]+forward[i][1];
                    //cerr<<"\t\t"<<forward[i+1][0]<<"\t"<<forward[i+1][1]<<"\t"<<gamma[i+1][i+1-last_clk[i]]<<"\t"<<alpha[id]<<"\t"<<beta[vid]<<endl;
                    last_clk[i+1]=last_clk[i];
                    clk_cnt[i+1]=clk_cnt[i];
                }
            }
            ...
            for(int i=DOCPERPAGE;i;--i)
            {
                ...
                if(sess.click_time[i]>.1)
                {
                    backward[i-1][0]=(backward[i][0]*(1.-s_c[id])+backward[i][1]*s_c[id])*gamma[i][i-last_clk[i-1]][clk_cnt[i-1]]*alpha[id]*beta[vid];
                }
                else
                {
                    backward[i-1][0]=backward[i][0]*(1.+gamma[i][i-last_clk[i-1]][clk_cnt[i-1]]*alpha[id]*((1.-beta[vid])*(1.-s_e[id])-1.))\
                        +backward[i][1]*gamma[i][i-last_clk[i-1]][clk_cnt[i-1]]*alpha[id]*(1.-beta[vid])*s_e[id];
        ...
        void get_click_prob(Session &sess,double* click_prob)
        {
            ...
            for(int i=1;i<=DOCPERPAGE;++i)
            {
                ...
                if(sess.click_time[i]>.1)
                {
                    click_prob[i]=forward[i-1][0]/(forward[i-1][0]+forward[i-1][1])*gamma[i][i-last_clk][clk_cnt]*alpha[id]*beta[vid];
                    last_clk=i;
                    if(vid==1)
                        clk_cnt|=1;
                    else
                ...
                {
                    click_prob[i]=1.-forward[i-1][0]/(forward[i-1][0]+forward[i-1][1])*gamma[i][i-last_clk][clk_cnt]*alpha[id]*beta[vid];
                }
            }
            /*for(int i=1;i<=DOCPERPAGE;++i)
            {
