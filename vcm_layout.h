    ...
    public:
        ...
        void update(int rnd)
        {
            //if(rnd%2==1)
            {
                ...
                for(int i=0;i<=DOCPERPAGE;++i)
                {
                    ...
                    {
                        update_num(gamma[s][p][i][j],cgamma[s][p][i][j]);
                    }
                }
                //return;
            }
        ...
        void get_3prob(Session &sess,double &p1,double &p2,double &p3,int v)
        {
            ...
            for(int i=v;i;--i)
            {
                ...
                {
                    p1*=rge((beta[sess.doc_id[i]]))*rge((gamma[v][1][num][num-last_clk]+cita[sess.doc_id[i]]));
                    last_clk=num;
                }
                else
                {
                    p1*=1.-rge((beta[sess.doc_id[i]]))*rge((gamma[v][1][num][num-last_clk]+cita[sess.doc_id[i]]));
                }
                ++num;
            }
            ...
            {
                p2*=rge((beta[sess.doc_id[v]]))*rge((gamma[v][0][num][num-last_clk]+cita[sess.doc_id[v]]));
                last_clk=num;
            }
            else
            {
                p2*=1.-rge((beta[sess.doc_id[v]]))*rge((gamma[v][0][num][num-last_clk]+cita[sess.doc_id[v]]));
            }
            ++num;
            for(int i=1;i<=DOCPERPAGE;++i)
                if(i!=v)
                {
                    if(sess.click_time[i]>.1)
                    {
                        p2*=rge((beta[sess.doc_id[i]]))*rge((gamma[v][0][num][num-last_clk]+cita[sess.doc_id[i]]));
                        last_clk=num;
                    }
                    else
                    {
                        p2*=1.-rge((beta[sess.doc_id[i]]))*rge((gamma[v][0][num][num-last_clk]+cita[sess.doc_id[i]]));
                    }
                    ++num;
                }
            ...
            if(sess.click_time[v]>.1)
            {
                p3*=alpha[sess.doc_id[v]]*gamma[0][0][num][num-last_clk];
                last_clk=num;
            }
            else
            {
                p3*=1.-alpha[sess.doc_id[v]]*gamma[0][0][num][num-last_clk];
            }
            ++num;
            for(int i=1;i<=DOCPERPAGE;++i)
                if(i!=v)
                {
                    if(sess.click_time[i]>.1)
                    {
                        p3*=alpha[sess.doc_id[i]]*gamma[0][0][num][num-last_clk];
                        last_clk=num;
                    }
                    else
                    {
                        p3*=1.-alpha[sess.doc_id[i]]*gamma[0][0][num][num-last_clk];
                    }
                    ++num;
                }
        }
        void calc(Session &sess,double &p,double &ptot,int &i,int &last_clk,int &num,int v,int typ)
        {
            ...
            gam=gamma[s1][p1][num][num-last_clk];
            ...
            if(sess.click_time[i]>.1)
            {
                ...
                {
                    cgamma[s1][p1][num][num-last_clk]+=p/rge(gam+cit)/ptot;
            ...
            else
            {
                ...
                if(/*gam+cit<1.-2*eps&&*/gam+cit>2*eps)
                {
                    cgamma[s1][p1][num][num-last_clk]-=p/(1.-rge(bet)*rge(gam+cit))*rge(bet)/ptot;
        ...
        void calc2(Session &sess,double &p,double &ptot,int &i,int &last_clk,int &num)
        {
            ...
            alp=alpha[sess.doc_id[i]];
            gam=gamma[0][0][num][num-last_clk];
            if(sess.click_time[i]>.1)
            {
                calpha[sess.doc_id[i]]+=p/(alp)/ptot;
                cgamma[0][0][num][num-last_clk]+=p/gam/ptot;
                last_clk=num;
            }
            else
            {
                calpha[sess.doc_id[i]]-=p/(1.-alp*gam)*(gam)/ptot;
                cgamma[0][0][num][num-last_clk]-=p/(1.-(alp)*(gam))*(alp)/ptot;
            }
            ++num;
        }
        ...
        void train()
        {
            ...
            for(rnd=1;rnd<=MAXROUND;++rnd)
            {
                    ...
                    if(first_vertical[idx]==0)
                    {
                        for(int i=1;i<=DOCPERPAGE;++i)
                            if(sess.click_time[i]>.1)
                            {
                                ...
                                cgamma[0][0][i][i-last_clk]+=1./gamma[0][0][i][i-last_clk];
                                last_clk=i;
                            }
                            else
                            {
                                calpha[sess.doc_id[i]]-=gamma[0][0][i][i-last_clk]/(1.-alpha[sess.doc_id[i]]*gamma[0][0][i][i-last_clk]);
                                cgamma[0][0][i][i-last_clk]-=alpha[sess.doc_id[i]]/(1.-alpha[sess.doc_id[i]]*gamma[0][0][i][i-last_clk]);
                            }
                    }
                    else
                    {
                        ...
                        num=1;
                        for(int i=v;i;--i)
                            calc(sess,p1,ptot,i,last_clk,num,v,1);
                        for(int i=v+1;i<=DOCPERPAGE;++i)
                            calc(sess,p1,ptot,i,last_clk,num,v,1);
                        //F1,B0
                        last_clk=0;
                        num=1;
                        calc(sess,p2,ptot,v,last_clk,num,v,0);
                        for(int i=1;i<=DOCPERPAGE;++i)
                            if(i^v)
                                calc(sess,p2,ptot,i,last_clk,num,v,0);
                        //F0
                        last_clk=0;
                        num=1;
                ...
                if(rnd%20==0)
                {
                    ...
                    cerr<<"Round = "<<rnd<<"\tTest = "<<now_LL<<"\tDelta = "<<delta<<"\t"<<sigma[2][4]<<endl;
                }
        ...
        void get_click_prob(Session &sess,double* click_prob)
        {
            ...
            if(v>DOCPERPAGE)
            {
                for(int i=1;i<=DOCPERPAGE;++i)
                {
                    if(sess.click_time[i]>.1)
                    {
                        click_prob[i]=alpha[sess.doc_id[i]]*gamma[0][0][i][i-last_clkk];
                        last_clkk=i;
                    }
                    else
                    {
                        click_prob[i]=1.-alpha[sess.doc_id[i]]*gamma[0][0][i][i-last_clkk];
                    }
                }
                return;
            }
            ...
            if(sess.click_time[i]>.1)
            {
                click_prob[i]+=c3*((alpha[sess.doc_id[i]])*(gamma[0][0][num][num-last_clkk]));
                last_clkk=num;
            }
            else
            {
                click_prob[i]+=c3*(1.-(alpha[sess.doc_id[i]])*(gamma[0][0][num][num-last_clkk]));
            }
            ++num;
            for(i=1;i<=DOCPERPAGE;++i)
                if(i^v)
